...
| `ManualEntriesPage` | `/subs-manual` | Manual order entry → push to subscription KDS |
...
| 2026-09-16 | Fix 64 false-positive TypeScript errors in IDE Problems tab: Enabled Deno in `.vscode/settings.json` (`deno.enable: true`, `deno.importMap`), disabled VS Code's built-in TypeScript validator (`typescript.validate.enable: false`) which was falsely flagging Deno URL imports and global `Deno` namespace across all Supabase edge functions, and added Deno extension recommendation in `.vscode/extensions.json`. | `.vscode/settings.json`, `.vscode/extensions.json` |
| 2026-09-16 | **Admin Android APK**: (1) Built admin app as Android APK (`com.protibowls.admin`, app name "PROTI BOWLS ADMIN"). (2) **Password-protected exit** — back/exit now requires the employee's Supabase login password via re-authentication; prevents unauthorized exits on shared tablets. (3) **Native Android notifications** — replaced Firebase (web-only) with Supabase Realtime + `flutter_local_notifications` on Android; subscribes to `orders` table for new pending orders and fires local notifications with sound + vibration. (4) Firebase init wrapped in `kIsWeb` check. (5) `PushNotificationService` refactored to conditional imports (`_web.dart` for FCM, `_io.dart` for Realtime + local). (6) App icon generated from Proti Bowls logo via `flutter_launcher_icons`. (7) Enabled core library desugaring in Gradle. `flutter analyze` clean (0 issues). APK: 57.8MB at `admin/build/app/outputs/flutter-apk/app-release.apk`. | `main.dart`, `exit_guard.dart`, `push_notification_service.dart` (→conditional export), `push_notification_service_web.dart` [NEW], `push_notification_service_io.dart` [NEW], `pubspec.yaml`, `build.gradle.kts`, `AndroidManifest.xml`, `ARCHITECTURE.md` |
| 2026-09-16 | **Manual Entries page for subscription model**: (1) New full-screen page at `/subs-manual` where a manager selects/adds a customer, picks dishes from the subscription meal library, sets plan pricing, chooses COD/Prepaid payment, and adds special instructions. (2) **Direct push to KDS** — entries bypass Today's Meal review and appear on the subscription kitchen display immediately with per-dish timer. (3) **Special instructions in RED BOLD** on subscription KDS — `reply_text` and `notes` now display in a red-bordered container with `FontWeight.w800` and `Colors.red[900]` instead of the previous indigo italic. (4) Provider enhanced with `fetchExistingCustomers()` (deduped from manual_subscription_entries + orders) and `addManualEntry()` now accepts `payment_method` (cod/prepaid) and `auto_push` flag. `flutter analyze` clean (0 issues). | `manual_entries_page.dart` [NEW], `subscription_admin_provider.dart`, `subscription_chef_page.dart`, `subscription_dashboard_page.dart`, `router.dart`, `ARCHITECTURE.md` |
| 2026-09-16 | **Category-wise filter & search in Manual Entries**: (1) Added category tabs (`ALL`, `CHICKEN ITEMS`, `PANEER (VEG) ITEMS`, `EGG ITEMS`, `SNACKS`, etc.) with dish counts on the Manual Entries page. (2) Added instant dish search bar with clear button. (3) Enhanced `SubscriptionAdminProvider.fetchMealLibrary()` to dynamically map category and badge from `menu_items` for every subscription meal, exposing `mealCategories`. (4) Each dish card in the grid now displays its category name and veg/non-veg badge. `flutter analyze` clean (0 issues). | `manual_entries_page.dart`, `subscription_admin_provider.dart`, `ARCHITECTURE.md` |
| 2026-09-16 | **Manual Entries Amount Field Refinements**: (1) Amount field now strictly enforces **integer only** using `FilteringTextInputFormatter.digitsOnly` and `int.tryParse`. (2) **Prepaid orders do NOT mandate amount** — amount is completely optional for prepaid, defaulting to 0 without blocking. COD orders still enforce entering collection amount. `flutter analyze` clean (0 issues). | `manual_entries_page.dart`, `ARCHITECTURE.md` |
| 2026-09-16 | **Schema-Resilient addManualEntry**: Fixed PostgrestException PGRST204 when `payment_method` column is missing in `manual_subscription_entries`. Provider now catches missing column and re-inserts with payment method prepended to `notes` (`[COD]`/`[PREPAID]`). Added try-catch fallback for `meal_confirmations` insert. Added Section 8 migration to `MIGRATION_MANUAL_ENTRY_KDS.sql`. `flutter analyze` clean (0 issues). | `subscription_admin_provider.dart`, `MIGRATION_MANUAL_ENTRY_KDS.sql`, `ARCHITECTURE.md` |
| 2026-09-16 | **Subscription KDS "TO PREPARE" Count & Sound Removal**: (1) Fixed `toPrepare` stat count in `subscription_chef_page.dart` to calculate remaining unprepared meals (`totalMeals - prepared`), displaying **0** when all meals are prepared instead of the total. (2) Stat cards updated to `TO PREPARE`, `PREPARED`, `TOTAL`. (3) `byPref` chip breakdown now only counts remaining meals to prepare, hiding `OTHER × 4` when 0 meals remain. (4) Completely removed order confirmation sound and orange `CONFIRM (STOP SOUND)` button from subscription KDS (gym KDS untouched). `flutter analyze` clean (0 issues). | `subscription_chef_page.dart`, `ARCHITECTURE.md` |

...
// Creates a Razorpay order for a Gold membership PLAN. The price comes from
// the DB (gym_membership_plans) — the client only sends a plan id, never an
// amount. A skeleton `gym_memberships` row (awaiting_payment) is stored so
// the payment can be reconciled even if the customer abandons the
// password-setup step (see gym-membership-activate).
...
serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      await sb.from("order_rate_limit").insert({ ip, phone: "gym-membership" });
    } catch (_) { /* fail open */ }
    ...
    const rzpRes = await fetch("https://api.razorpay.com/v1/orders", {
      ...
      body: JSON.stringify({
        ...
        payment_capture: 1,
        notes: { type: "gym_membership", plan: plan.name },
      }),
    });
    ...
    const { data: row, error: insErr } = await sb.from("gym_memberships").insert({
      plan_id: plan.id,
      status: "awaiting_payment",
      razorpay_order_id: rzp.id,
    }).select("id").single();
    if (insErr) return json({ error: "Could not start membership" }, 500);
    ...
    console.error("gym-membership-create-order error:", e);
